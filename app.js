...
const { AccountInfo } = require('@aws-sdk/client-sso')
...
const updateCredentials = async () => {
    ...
    const devAccountList = filterByDevelopmentEnvironment(accountList);

    for (const { accountId, accountName } of devAccountList) {
        ...
        for (const { roleName } of roleList) {
            ...
            const account_section_name = 'default'
...
filterByDevelopmentEnvironment = (accountList) => {
    const devAccountList = accountList.filter(param => param.accountName == "DEV");
